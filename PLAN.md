...
- [x] *(Done 2026-07-10.)* TomTom developer account → API key. Free tier: **2,500 requests/day** (verified Jul 2026). Endpoint: **Traffic API → Flow Segment Data** (`/traffic/services/4/flowSegmentData/...`) — takes a lat/lon, returns nearest segment's `currentSpeed`, `freeFlowSpeed`, `currentTravelTime`, `freeFlowTravelTime`, `confidence`, `roadClosure`.
...
- [x] GitHub repo `jam-genome` (public from day 1). *(Live 2026-07-09: github.com/KrishSachdev/jam-genome.)*
...
- *(Tooling built 2026-07-14: `analysis/episodes.py` (episode extraction + threshold sensitivity, reused by Phase 3) and `analysis/eda.py` (league table CSV, heatmap, weekday/weekend profiles).)*
